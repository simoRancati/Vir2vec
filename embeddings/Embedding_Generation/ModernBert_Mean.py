"""Extract **mean‑pooled** embeddings with RaphaelMourad/ModernBert‑DNA‑v1‑37M‑virus.

✓ Loads tokenizer + base model (`AutoModel`).
✓ Saves one HDF5:  `embeddings_results_modern/ModernBert_mean_embeddings.h5`.
✓ Prints progress; NaN‑guard prints the first batch only.
"""
...
OUTPUT_DIR = Path('embeddings_results_modern'); OUTPUT_DIR.mkdir(exist_ok=True)
MODEL_ID   = 'RaphaelMourad/ModernBert-DNA-v1-37M-virus'
FILE_OUT   = OUTPUT_DIR / 'ModernBert_mean_embeddings.h5'
BATCH_SIZE = 16
CHUNK_SIZE = 200
MAX_LEN    = 4096
USE_FP16   = False
...
def embed_batch(model, tok, seqs):
    tok_out = tok(seqs, padding=True, truncation=True, max_length=MAX_LEN,
                  return_tensors='pt', return_overflowing_tokens=True, stride=512)
    mapping = tok_out.pop('overflow_to_sample_mapping', None)
    ids  = tok_out['input_ids'].to(model.device)
    mask = tok_out['attention_mask'].to(model.device)
        ...
        for idx in idxs:
            with torch.no_grad():
                with autocast(enabled=USE_FP16):
                    out = model(input_ids=ids[idx:idx+1], attention_mask=mask[idx:idx+1])
                    chunks.append(out.last_hidden_state.mean(dim=1).cpu())
        embs.append(torch.mean(torch.cat(chunks), dim=0).numpy())
...
def main():
    ...
    print('Loading ModernBert‑DNA model…')
    model = AutoModel.from_pretrained(MODEL_ID, torch_dtype=torch.float32, device_map={'':0}).eval()
