...
#############################
CSV_PATH = '/blue/simone.marini/share/UseCases/covid/unique/omicron/newseqs.csv'
OUTPUT_DIR = '/blue/salemi/share/varcovid/ViralLingo/Embedding/UseCases/Covid_Bert/embeddings_results_covid_new'
CACHE_DIR  = '/blue/salemi/share/varcovid/ViralLingo/Embedding/ModelCache'   # <-- NEW
CHUNK_SIZE = 50
BATCH_SIZE = 4
...
MODELS = [
    {
        "name": "Modern-Bert_Virus",  # Custom locally trained model
        "model_path": 'RaphaelMourad/ModernBert-DNA-v1-37M-virus',
        "model_class": AutoModel,  # Uses a model class for causal generation
        "tokenizer_class": AutoTokenizer,
        "trust_remote_code": True,  # Do not trust remote code execution by default
        ...
        try:
            ...
            for j in range(len(batch_sequence)):
                ...
                if 'overflow_to_sample_mapping' in inputs:
                    seq_indices = [idx for idx, sample_idx in enumerate(inputs.overflow_to_sample_mapping) if
                                   sample_idx == j]
                else:
                    seq_indices = [j]
