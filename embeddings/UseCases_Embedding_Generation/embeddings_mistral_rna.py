...
#############################
CSV_PATH = '/blue/simone.marini/share/rna_dna/rna.csv'
OUTPUT_DIR = 'embeddings_results_mistral_rna'
CHUNK_SIZE = 50
BATCH_SIZE = 4
...
def get_embeddings_batched(model, tokenizer, sequences, accession, batch_size=BATCH_SIZE):
    results = []
    ...
    for i in range(0, len(sequences), batch_size):
        ...
        batch_accession = accession[i: i + batch_size]
...
def process_embeddings(model_info):
    ...
    all_embeddings, all_accession = [], []
    total_rows = sum(1 for _ in open(CSV_PATH)) - 1
    rows_processed = 0

    for chunk_idx, chunk in enumerate(pd.read_csv(CSV_PATH, chunksize=CHUNK_SIZE)):
        if 'Sequence' not in chunk.columns or 'accession' not in chunk.columns:
            raise ValueError("CSV must contain 'Sequence' and 'accession' columns.")

        sequences = chunk['Sequence'].tolist()
        accession = chunk['accession'].tolist()
        print(f"Processing chunk {chunk_idx + 1} containing {len(sequences)} sequences …")

        embeddings = get_embeddings_batched(model, tokenizer, sequences, accession)

        for emb in embeddings:
            all_embeddings.append(emb["Embedding"])
            all_accession.append(emb["accession"])
            ...
            if (chunk_idx + 1) % 5 == 0:
                ...
                save_embeddings(all_embeddings, all_accession, embeddings_file)
