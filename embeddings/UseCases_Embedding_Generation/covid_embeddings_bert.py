...
CSV_PATH   = '/blue/simone.marini/share/Code_Mistral/covid/covid.csv'
OUTPUT_DIR = Path('embeddings_results_modern_covid'); OUTPUT_DIR.mkdir(exist_ok=True)
...
def main():
    ...
    # Count valid rows first (excluding nulls)
    print('Counting valid sequences...')
    valid_count = 0
    for chunk in pd.read_csv(CSV_PATH, chunksize=CHUNK_SIZE):
        valid_count += chunk['sequence'].notna().sum()
    print(f'Found {valid_count} valid sequences')

    # Create HDF5 datasets
    h5f   = h5py.File(FILE_OUT, 'w')
    emb_ds = h5f.create_dataset(
        'embeddings',
        shape=(valid_count, model.config.hidden_size),
    ...
    for cidx, chunk in enumerate(pd.read_csv(CSV_PATH, chunksize=CHUNK_SIZE)):
        # Filter out null/NaN sequences and corresponding labels
        valid_mask = chunk['sequence'].notna() & chunk['Variant'].notna()
        chunk_filtered = chunk[valid_mask]
        
        if len(chunk_filtered) == 0:
            print(f'Chunk {cidx + 1}: No valid sequences, skipping')
            continue
            
        seqs = chunk_filtered['sequence'].astype(str).tolist()  # Ensure strings
        labs = chunk_filtered['Variant'].astype(str).tolist()   # Ensure strings
        
        batches = math.ceil(len(seqs) / BATCH_SIZE)
        print(f'Chunk {cidx + 1}: {len(seqs)} valid seqs, {batches} batches')

        for b in range(batches):
            start_idx = b * BATCH_SIZE
            end_idx = min((b + 1) * BATCH_SIZE, len(seqs))
            ...
            lbl_ds[row_ptr:row_ptr + len(emb_mat)] = batch_labs
            row_ptr += len(emb_mat)

            pct = row_ptr / valid_count * 100
