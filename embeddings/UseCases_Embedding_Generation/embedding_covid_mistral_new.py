...
OUTPUT_DIR = '/blue/salemi/share/varcovid/ViralLingo/Embedding/UseCases/Covid_Mistral/embeddings_results_mistral_covid_new'
...
def process_embeddings(model_info):
    ...
    for chunk_idx, chunk in enumerate(pd.read_csv(CSV_PATH, chunksize=CHUNK_SIZE)):
        ...
        print(f"Processing chunk {chunk_idx + 1} containing {len(sequences)} sequence …")

        embeddings = get_embeddings_batched(model, tokenizer, sequences, Variant)
